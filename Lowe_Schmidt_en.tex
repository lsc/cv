\documentclass[8pt,sans]{article}
...
Lingv\"agen 184		& \textit{Phone:} +46 723 867 157\\
...
\begin{outerlist}
\item[] Infrastructure development and architecture, using AWS, Terraform, the HashiStack (Consul, Nomad, Vault), Golang and Bash.

\item Transformed a manual provisioned and static compute platform with multiple single points of failures to an automated, distributed platform based around the HashiStack, provisioned and managed with Ansible, Packer and Terraform.
\item Migrated a manually managed Percona Galera cluster to AWS Aurora, cutting down maintenance work from 10 of hours a week to minutes a month, moved TB's of data with no noticeable user impact.
\item Implemented CD for Terraform with Atlantis, allowing coworkers outside of the platform team to manage infrastructure independently and with autonomy.
\item Automated repetitive work like TLS certificate issuing, platform creation and destruction, AMI building and deployment of services.
...
\section{Languages}
Fluent in Swedish and English, conversational German.
\end{document}
