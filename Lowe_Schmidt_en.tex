...
\begin{outerlist}
\item[] Qapital is a neobank offering saving, spending and investment products together with our unique rules and goals approach to finances. 
...
\item Transformed a manually provisioned and static compute platform with lots of single point of failures to a automated, distributed platform based around the Hashicorp products. The platform went live in production November 2018 with only minor service disruptions.
...
\begin{outerlist}	
\item[] Learned the importance of version control (even) for system administrators (RCS and Subversion), racked and stacked machines and learned to plan my network design up front (because rearranging physical cables is exhausting, annoying and disrupting), consulted as application server admin for WebLogic and WebSphere and replaced an aging Windows 2000 AD server with a FLOSS solution (OpenLDAP, Samba, Kerberos).
\end{outerlist}
\blankline
...
\blankline
\begin{outerlist}
	\item A mixed bag of courses including (but not limited to) Sendmail, Bind, Apache and MySQL administration, programming for UNIX in C and Perl, networking in both the practical (switching, routing, TCP/IP, DNS) and theoretical (OSI layers) sense.
\end{outerlist}
\blankline
